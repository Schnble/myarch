...
local function command_from(value, description)
  ...
  if value.myarch then
    return "myarch-launch-" .. value.myarch
  ...
  elseif value.tui then
    if value.focus then
      return "myarch-launch-or-focus-tui " .. shell_quote(value.tui)
    else
      return "myarch-launch-tui " .. shell_quote(value.tui)
    end
  end
...
function o.launch_webapp(url)
  return "myarch-launch-webapp " .. shell_quote(url)
end

function o.launch_webapp_sole(name, url)
  return "myarch-launch-or-focus-webapp " .. shell_quote(name) .. " " .. shell_quote(url)
end

function o.launch_sole(match, command)
  return "myarch-launch-or-focus " .. shell_quote(match) .. " " .. shell_quote(o.launch(command))
end

function o.bind_menu(keys, description, menu, options)
  o.bind(keys, description, menu and ("myarch-menu " .. menu) or "myarch-menu", options)
end

function o.bind_toggle(keys, description, toggle, options)
  o.bind(keys, description, "myarch-toggle-" .. toggle, options)
end
