--
-- Dynamic Myarch Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the myarch-shipped Plymouth via
-- myarch-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs myarch-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "myarchunlocks"
NamePretty = "Myarch Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/myarch/themes"
  local myarch_path = os.getenv("MYARCH_PATH") or ""
  local default_themes_dir = myarch_path .. "/themes"
  local default_preview = myarch_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "myarch-launch-floating-terminal-with-presentation "
                .. shell_escape("myarch-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "myarch-launch-floating-terminal-with-presentation "
        .. shell_escape("myarch-plymouth-reset"),
    },
  }
