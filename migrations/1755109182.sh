...
# Users can still opt-in to Cloudflare DNS using: myarch-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-myarch-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-myarch-dns.network
  ...
  echo "To use Cloudflare DNS, run: myarch-setup-dns Cloudflare"
fi