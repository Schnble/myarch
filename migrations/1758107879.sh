...
for pkg in "${PACKAGES[@]}"; do
  if ! myarch-pkg-present "$pkg"; then
...
if $NEEDS_MIGRATION; then
  ...
  myarch-pkg-drop walker-bin walker-bin-debug

  myarch-pkg-add "${PACKAGES[@]}"

  source $MYARCH_PATH/install/config/walker-elephant.sh

  rm -rf ~/.config/walker/themes

  myarch-refresh-config walker/config.toml
  myarch-refresh-config elephant/calc.toml
  myarch-refresh-config elephant/desktopapplications.toml
fi
