...
# Move user-added backgrounds from Myarch theme folders to user config
MYARCH_DIR="$HOME/.local/share/myarch"
USER_BACKGROUNDS_DIR="$HOME/.config/myarch/backgrounds"

if [[ -d $MYARCH_DIR/themes ]]; then
  cd "$MYARCH_DIR"

  # Get list of git-tracked background files (relative to myarch dir)
...
THEMES_DIR="$HOME/.config/myarch/themes"
CURRENT_THEME_LINK="$HOME/.config/myarch/current/theme"
...
elif [[ -f $HOME/.config/myarch/current/theme.name ]]; then
  CURRENT_THEME_NAME=$(cat "$HOME/.config/myarch/current/theme.name")
fi

# Remove all symlinks from ~/.config/myarch/themes
...
if [[ -n $CURRENT_THEME_NAME ]]; then
  myarch-theme-set "$CURRENT_THEME_NAME"
else
