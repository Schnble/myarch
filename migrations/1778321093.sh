echo "Migrate Hyprland user config from Myarch hyprlang files to Lua files"

hypr_config_dir="$HOME/.config/hypr"
myarch_hypr_config_dir="$MYARCH_PATH/config/hypr"
toggle_state_dir="$HOME/.local/state/myarch/toggles/hypr"
toggle_defaults_dir="$MYARCH_PATH/default/hypr/toggles"
timestamp=$(date +%s)
...
for lua_config in "$myarch_hypr_config_dir"/*.lua "$myarch_hypr_config_dir"/.luarc.json; do
...
if [[ -f $sddm_hypr_conf || -f $sddm_wayland_conf ]]; then
  if [[ -f $MYARCH_PATH/default/sddm/hyprland.lua ]]; then
    sudo mkdir -p /usr/share/sddm
    sudo cp "$MYARCH_PATH/default/sddm/hyprland.lua" "$sddm_hypr_lua"
  fi
