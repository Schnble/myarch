...
echo "Migrating to use myarch-launch-webapp and myarch-launch-browser"
for desktop_file in ~/.local/share/applications/*.desktop; do
  if grep -q 'Exec=chromium --new-window --ozone-platform=wayland --app=' "$desktop_file"; then
    ...
    if [[ -n $url ]]; then
      sed -i "s|^Exec=.*|Exec=myarch-launch-webapp \"$url\"|" "$desktop_file"
    fi
  fi
done
...
if [[ -f $HYPR_BINDINGS_FILE ]]; then
  sed -i 's/\$browser =.*chromium.*$/\$browser = myarch-launch-browser/' "$HYPR_BINDINGS_FILE"
  sed -i 's/\$webapp="/myarch-launch-webapp "/g' "$HYPR_BINDINGS_FILE"
